...
int checktoken(char c[32]);

int checktoken(char b[30]){

}



int checkchannel(char b[30]){
FILE *chn = fopen("usernames.txt" , "r");
while(1){
char c;
char ch[100];
c = fgetc(chn);
...
y = cJSON_Create_String("Error");
z = cJSON_CreateString("The token is unvalid");
...
z = cJSON_CreateString("The channel name is already taken");
buffer = cJSON_Print(x);
		// Send the buffer to client
		send(client_socket, buffer, sizeof(buffer), 0);
